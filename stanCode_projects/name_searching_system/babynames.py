    ...
    with open(filename, 'r') as f:
        for line in f:
            if line.find(',') == -1:
                ...
                name1 = baby_list[1].strip()
                name2 = baby_list[2].strip()
...
def search_names(name_data, target):
    ...
    # print(name_lst)
    matching_names = check_the_word(target, name_lst)
    return matching_names


def check_the_word(target, name_lst):
    ans = []
    for i in range(len(name_lst)):
        if target.lower() in name_lst[i].lower():
            ans.append(name_lst[i])
    return ans
